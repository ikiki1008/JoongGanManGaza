...
String sql = "select title, content from qna where num='"+num+"'";
...
<body>
	...
	<div class="container">
		<div class="row">
			<form method="post" action="qnaUpdatePro.jsp">
				<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				<thead>
					<tr>
						<th colspan="2" style="background-color: #eeeeee; text-align: center;">문의사항 글수정</th>						
					</tr>
				</thead>
				<tbody>
					<tr>
						<td><input type="text" id="title" name="bbsTitle" class="form-control" value="<%=title %>" maxlength="50" required="required"></td>
					</tr>
					<tr>
						<td><textarea id="content" name="bbsContent" class="form-control" maxlength="2048" style="height: 350px" required="required"><%=content %></textarea></td>
						<input type="hidden" id="num" name="num" value="<%=num%>">					
					</tr>
				</tbody>
			</table>
			<input type="button" style="margin:3px;" class="btn btn-primary pull-right" onclick="location='qna.jsp'" value="취소">
			<input type="submit" style="margin:3px;" class="btn btn-primary pull-right" onclick="checkboard()" value="수정">
			</form>						
		</div>
	</div>
		<br>
...
function checkboard(){
	...
	if(ctitle.value!=""&&ccontent.value!=""){
		location.href='qnaUpdatePro.jsp?title='+(ctitle.value)+'+&content='+(ccontent.value);
